...
const Header = () => {
  ...
  const handleLogout = async () => {
    if (currentRide && user.role === "captain") {
  ...
  return (
    <>
            ...
            {isDropdownOpen && (
              <div className="absolute right-0 mt-2 w-[calc(100vw-2rem)] sm:w-72 rounded-xl bg-black/95 backdrop-blur-xl border border-white/10 shadow-2xl p-3 sm:p-4 space-y-3 z-50">
                ...
                <div className="space-y-1">
                  {!currentRide ? (
                    // Show normal menu items when no active ride
                    <>
                      {user.role === "user" ? (
                        <button
      ...
      <ConfirmModal
        ...
        message="Logging out will cancel the ongoing ride and may affect your captain rating. Are you sure you want to continue?"
      />
    </>
  );
};
